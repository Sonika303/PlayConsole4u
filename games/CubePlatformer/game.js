// game.js — engine (requires levels.js)
...
.tb{width:58px;height:58px;background:rgba(255,255,255,.1);
...
function respawn(){
  ...
  timerStart=performance.now()-elapsedMs; // keep timer running on respawn
}
...
function initGame(lvlNum){
  ...
  cam={x:0,y:0};timerStart=performance.now();elapsedMs=0;
  setupTouch();requestAnimationFrame(loop);
}
...
function loop(){
  ...
  player.dx*=0.83;player.x+=player.dx;
  player.prevY=player.y;player.dy+=G;player.y+=player.dy;
  player.x=Math.max(0,Math.min(player.x,lvl.w-player.w));
  if(player.y>lvl.h+60){player.x=player.sx;player.y=player.sy;player.dx=0;player.dy=0;player.jumps=0;player.onWall=false;player.onGround=false;}

  player.onWall=false;player.onGround=false;

  lvl.platforms.forEach(p=>{
    if(player.x+player.w<=p.x||player.x>=p.x+p.w||player.y+player.h<=p.y||player.y>=p.y+p.h)return;
    ...
    }else if(player.dy<0&&wasBelow){player.y=p.y+p.h;player.dy=0;}
    else{player.onWall=true;player.wallDir=(player.x+player.w/2<p.x+p.w/2)?1:-1;player.dx=0;player.dy*=0.75;}
  });

  if(!player.onGround){
    player.rotation+=player.spinSpeed;player.spinSpeed*=0.98;
  ...
  lvl.hazards.forEach(h=>{
    if(player.x+player.w>h.x&&player.x<h.x+h.w&&player.y+player.h>h.y&&player.y<h.y+h.h){
      player.x=player.sx;player.y=player.sy;player.dx=0;player.dy=0;player.jumps=0;
    }
  });
  ...
  if(player.x+player.w>g.x&&player.x<g.x+g.w&&player.y+player.h>g.y&&player.y<g.y+g.h){
    ...
    if(u){
      window.FB.getProfile(u.uid).then(profile=>{
        ...
        window.FB.saveTime(u.uid,'CubePlatformer',currentLvl,secs);
        // Update photoURL in profile if not set
        if(!profile.photoURL&&u.photoURL){
          window.FB.saveProfile(u.uid,{photoURL:u.photoURL,username});
        }
      });
    }
    drawWin(secs);return;
...
function draw(lvl){
  ...
  lvl.platforms.forEach(p=>{
    ctx.fillStyle=p.c;ctx.beginPath();ctx.roundRect(p.x,p.y,p.w,p.h,5);ctx.fill();
    ctx.fillStyle='rgba(255,255,255,.14)';ctx.beginPath();
    ctx.roundRect(p.x+3,p.y+2,p.w-6,Math.min(5,p.h/2),3);ctx.fill();
  });

  lvl.hazards.forEach(h=>{
    ctx.fillStyle='#7f1d1d';ctx.beginPath();ctx.roundRect(h.x,h.y,h.w,h.h,3);ctx.fill();
    ctx.fillStyle='#ef4444';
    const n=Math.floor(h.w/14);
    for(let i=0;i<n;i++){const sx=h.x+i*14+7;ctx.beginPath();ctx.moveTo(sx-5,h.y);ctx.lineTo(sx,h.y-9);ctx.lineTo(sx+5,h.y);ctx.closePath();ctx.fill();}
  });
  ...
  ctx.fillStyle='rgba(0,0,0,.2)';ctx.beginPath();ctx.ellipse(2,hh+4,hw+2,4,0,0,Math.PI*2);ctx.fill();
  ctx.rotate(player.rotation);
  ctx.fillStyle=lvl.accent;ctx.beginPath();ctx.roundRect(-hw,-hh,player.w,player.h,4);ctx.fill();
  ctx.fillStyle='rgba(255,255,255,.35)';ctx.beginPath();ctx.roundRect(-hw+3,-hh+3,hw-2,hh-2,3);ctx.fill();
  ...
  ctx.fillStyle='rgba(0,0,0,.6)';ctx.beginPath();ctx.roundRect(10,10,300,32,9);ctx.fill();
...
function drawWin(secs){
  const lvl=LEVELS[currentLvl];
  ctx.fillStyle='rgba(0,0,0,.9)';ctx.fillRect(0,0,VW,VH);
  // Glow circle
  const grd=ctx.createRadialGradient(VW/2,VH/2,0,VW/2,VH/2,180);
  grd.addColorStop(0,lvl.accent+'33');grd.addColorStop(1,'transparent');
  ctx.fillStyle=grd;ctx.fillRect(0,0,VW,VH);
  ctx.fillStyle=lvl.accent;ctx.font='bold 48px Nunito,sans-serif';ctx.textAlign='center';
  ctx.shadowColor=lvl.accent;ctx.shadowBlur=24;
  ctx.fillText('✓  LEVEL CLEAR!',VW/2,VH/2-56);ctx.shadowBlur=0;
  ctx.fillStyle='#fff';ctx.font='bold 24px Nunito,sans-serif';
  ctx.fillText('⏱  '+secs.toFixed(3)+'s',VW/2,VH/2-12);
  ctx.fillStyle='rgba(255,255,255,.6)';ctx.font='bold 15px Nunito,sans-serif';
  ctx.fillText(currentLvl>=MAX_LEVELS?'🎉 YOU BEAT ALL 10 LEVELS!':
    'Level '+(currentLvl+1)+' unlocked ★',VW/2,VH/2+24);
  ctx.fillStyle='rgba(255,255,255,.3)';ctx.font='12px Nunito,sans-serif';
