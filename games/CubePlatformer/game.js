...
// ── SKIN DEFINITIONS ──────────────────────────────────────────────────────────
const SKINS = {
  default: { body: null,              eye: '#fff',   pupil: 'rgba(0,0,0,.55)', shine: 'rgba(255,255,255,.35)', label: 'Default'  },
  ghost:   { body: 'rgba(220,230,255,.75)', eye: '#c4b5fd', pupil: 'rgba(60,20,120,.6)', shine: 'rgba(255,255,255,.5)',  label: 'Ghost'    },
  neon:    { body: '#00ff88',         eye: '#fff',   pupil: '#004422',         shine: 'rgba(255,255,255,.5)',  label: 'Neon'     },
  fire:    { body: '#ff4400',         eye: '#ffe066',pupil: '#7a2000',         shine: 'rgba(255,200,50,.45)', label: 'Fire'     },
  void:    { body: '#0d0d1a',         eye: '#818cf8',pupil: 'rgba(0,0,0,.9)', shine: 'rgba(130,140,250,.35)', label: 'Void'     },
  rainbow: { body: null, rainbow:true, eye: '#fff',  pupil: '#333',           shine: 'rgba(255,255,255,.4)',  label: 'Rainbow'  }
};
...
.tb{width:60px;height:60px;background:rgba(255,255,255,.1);border:2px solid rgba(255,255,255,.25);
  border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;
  color:#fff;pointer-events:all;user-select:none;-webkit-user-select:none}
...
async function initGame(lvlNum) {
  ...
  lvl.platforms.forEach(p => {
    if (p.moving) {
      p._t   = 0;
      p._ox  = p.x;   // original x
      p._oy  = p.y;   // original y
    }
  });
...
function resetPlayer() {
  ...
  timerStart = performance.now() - elapsedMs;  // keep timer running
}
...
function loop() {
  ...
  lvl.platforms.forEach(p => {
    if (!p.moving) return;
    p._t += p.moving.speed * 0.016;                      // ~1 tick at 60fps
    ...
    // Carry player standing on top
    if (player.onGround) {
      const wasOnThis =
        player.x + player.w > prevPx && player.x < prevPx + p.w &&
  ...
  for (const b of lvl.bouncePads) {
    if (player.x + player.w > b.x && player.x < b.x + b.w &&
        player.y + player.h > b.y && player.y + player.h < b.y + b.h + 12 &&
        player.dy > 0) {
      player.dy = -(b.force);
      player.y  = b.y - player.h;
      player.jumps = 0;
      player.spinSpeed = (player.dx > 0 ? 1 : -1) * 0.3;  // big spin on bounce
    }
  }
...
function draw(lvl) {
  ...
  for (const p of lvl.platforms) {
    ...
    // Moving indicator — pulsing edge glow
    if (p.moving) {
...
function drawHUD(lvl) {
  ...
  ctx.beginPath(); ctx.roundRect(10, 10, 290, 30, 8); ctx.fill();
  ctx.fillStyle = lvl.accent; ctx.font = 'bold 11px Nunito,sans-serif';
  ctx.fillText(`LVL ${currentLvl}/${MAX_LEVELS}  ·  ⏱ ${t}s  ·  SKIN: ${(SKINS[activeSkin]||SKINS.default).label}`, 18, 30);
}
